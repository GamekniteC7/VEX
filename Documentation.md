...
> `main()` is special. It is not declared with `fn` or any visibility modifier. Any function named `main` elsewhere — such as `pub fn main()` — is treated as a regular function, not the entry point.
