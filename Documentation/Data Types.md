...
``` VEX
let b: array<i32, 6> = {0, 1, 2, 3, 4, 5};
let b: range<i32> = array[1..3];        // b = {1, 2, 3}
```
...
If the range is f32, than the number of decimal places is equal to the number of decimal places in the declaration:

``` VEX
let r: range<f32> = 0.0..0.3;       // r = {0.0, 0.1, 0.2, 0.3}
let r2: range<f32> = 0.00..0.03     // r2 = {0.00, 0.01, 0.02, 0.03}
```
...
| ------ | ----------- | -------------- |
| `.shift(by: T)` | Shifts the entire range by `by` (can be neg) | Yes |
| `.add(how_much: T)` | Extends the end of the range by `how_much` (can be neg) | Yes |
| `.contains(n: T)` | Returns `true` if the range contains `n` | No |
| `.try_get(position: uint)` | Returns the value at the given index (0-based) — panics if out of bounds | No |
| `.get(position: uint)` | Returns `Option<T>` — `()` if out of bounds | No |
